-- =====================================================================
-- FacturacionHSM · SCRIPT 02 · FUNCIONES RPC (versión FINAL)
-- =====================================================================
-- Incluye TODOS los fixes acumulados en las iteraciones:
--   * sp_ejecutar_produccion: transformación DIRECTA CRUDO->TERMINADO
--     (fix calidades mixtas + fix producción parcial).
--   * sp_registrar_venta: descuenta del inventario CENTRAL compartido
--     (Fase 1, sin distribución) + fechas en zona horaria Colombia.
--   * sp_registrar_abono: fecha en zona horaria Colombia.
...
-- ---------------------------------------------------------------------
-- Helper: aplica un movimiento y actualiza el saldo (con bloqueo de fila)
-- ---------------------------------------------------------------------
...
begin
  ...
  -- RESTA en el origen (con bloqueo para evitar carreras)
  ...
  -- Registra el movimiento (libro mayor inmutable)
...
-- ---------------------------------------------------------------------
-- sp_recibir_mercancia (recibo total o parcial)
-- ---------------------------------------------------------------------
...
begin
  perform 1 from public.compras
    where id = p_compra_id and tenant_id = v_tenant and estado in ('PENDIENTE','PARCIAL');
  if not found then raise exception 'COMPRA_NO_PENDIENTE'; end if;
  ...
  loop
    ...
    if not found then raise exception 'DETALLE_INVALIDO'; end if;
...
-- ---------------------------------------------------------------------
-- sp_ejecutar_produccion (FINAL: transformación DIRECTA CRUDO->TERMINADO)
-- Consume solo lo de la orden (soporta producción parcial) y usa la
-- calidad de ORIGEN al consumir (soporta segunda/merma).
-- ---------------------------------------------------------------------
...
begin
  ...
  loop
    select estado_origen_id, calidad_origen_id
      into v_origen
      from public.ordenes_produccion_detalle
      where orden_id = p_orden_id
        and variante_id = (v_res->>'variante_id')::uuid
      limit 1;

    if v_origen.estado_origen_id is null then
      raise exception 'VARIANTE_NO_EN_ORDEN';
    end if;
    ...
    perform public.fn_aplicar_movimiento(
      ...
      v_ubic_central, v_origen.estado_origen_id, v_origen.calidad_origen_id,
  ...
  update public.ordenes_produccion
     set estado = 'CERRADA', fecha_fin = (now() at time zone 'America/Bogota')::date
   where id = p_orden_id;
end;
$$;
...
-- ---------------------------------------------------------------------
-- sp_transferir_inventario (ENVIO/RETORNO) - se conserva para fase futura
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- sp_registrar_venta (FINAL: descuenta de CENTRAL + fechas Colombia)
-- ---------------------------------------------------------------------
...
begin
  ...
  select municipio into v_municipio from public.clientes where id = v_cliente and tenant_id = v_tenant;

  perform 1 from public.vendedores where id = v_vendedor and tenant_id = v_tenant;
  if not found then raise exception 'VENDEDOR_INVALIDO'; end if;

  select id into v_ubic_central from public.ubicaciones
    where tenant_id = v_tenant and tipo = 'CENTRAL' limit 1;
  if v_ubic_central is null then raise exception 'SIN_UBICACION_CENTRAL'; end if;

  select id into v_est_listo from public.estados_inventario
    where tenant_id = v_tenant and codigo = 'LISTO' limit 1;

  insert into public.ventas (tenant_id, vendedor_id, cliente_id, municipio, fecha, tipo_pago,
                             subtotal, descuento, total, created_by)
    values (v_tenant, v_vendedor, v_cliente, v_municipio, v_hoy, v_tipo, 0, v_desc, 0, v_user)
  ...
  loop
    ...
    perform public.fn_aplicar_movimiento(
      ...
      v_ubic_central, v_est_listo, (v_item->>'calidad_id')::uuid,
      null, null, null,
  ...
  insert into public.facturas (tenant_id, venta_id, numero, fecha)
    values (v_tenant, v_venta, v_num_fac, v_hoy)
  ...
  if v_tipo = 'CREDITO' then
    ...
    values
      (v_tenant, v_factura, v_cliente, v_vendedor, v_hoy, v_dias,
       v_hoy + v_dias, v_total, 0, v_total, 'PENDIENTE')
...
begin
  ...
  insert into public.abonos (tenant_id, cuenta_id, fecha, monto, forma_pago, comprobante_url, observacion, created_by)
    values (v_tenant, p_cuenta_id, v_hoy, p_monto, p_forma_pago, p_comprobante_url, p_observacion, v_user);
  ...
  update public.cuentas_por_cobrar
     set total_abonado = v_nuevo_abonado, saldo_pendiente = v_nuevo_saldo, estado = v_estado
...
-- ---------------------------------------------------------------------
-- sp_anular_venta (reversa inventario a CENTRAL + elimina cartera)
-- ---------------------------------------------------------------------
...
begin
  select * into v_venta
    from public.ventas
   where id = p_venta_id and tenant_id = v_tenant
   for update;
  if not found then raise exception 'VENTA_INVALIDA'; end if;
  if v_venta.estado = 'ANULADA' then raise exception 'VENTA_YA_ANULADA'; end if;

  select * into v_cuenta
    from public.cuentas_por_cobrar
   where factura_id = (select id from public.facturas where venta_id = p_venta_id)
   for update;
  if found and v_cuenta.total_abonado > 0 then
    raise exception 'TIENE_ABONOS';
  end if;

  -- Inventario central compartido: el reverso devuelve a CENTRAL/LISTO
  select id into v_ubic_central from public.ubicaciones
    where tenant_id = v_tenant and tipo = 'CENTRAL' limit 1;
  select id into v_est_listo from public.estados_inventario
    where tenant_id = v_tenant and codigo = 'LISTO' limit 1;

  for v_det in select * from public.ventas_detalle where venta_id = p_venta_id
  loop
    perform public.fn_aplicar_movimiento(
      v_tenant, 'REVERSO', v_det.variante_id,
      null, null, null,
      v_ubic_central, v_est_listo, v_det.calidad_id,
      v_det.cantidad, 'VENTA', p_venta_id, v_user);
  end loop;

  if v_cuenta.id is not null then
    delete from public.cuentas_por_cobrar where id = v_cuenta.id;
  end if;
...
-- ---------------------------------------------------------------------
-- sp_ajustar_inventario (ajuste manual auditable +/-)
-- ---------------------------------------------------------------------
