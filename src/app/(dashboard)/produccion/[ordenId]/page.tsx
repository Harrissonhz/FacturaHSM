...
// Muestra descripción larga (Producto / Color / Talla) en vez del SKU.
...
export default async function CompraDetallePage({
  ...
  const { data: detalle } = await supabase
    ...
    .select(
      "id, cantidad_solicitada, cantidad_recibida, costo_unitario, " +
  ...
  if (!compra) {
    return (
      <main>
        <div className="empty-state"><span className="emoji">🛒</span>No se encontró la compra.</div>
      </main>
    );
  }
  ...
  const lineas: any[] = (detalle ?? []).map((d: any) => {
    const prod = d.variantes?.productos?.nombre ?? d.variantes?.sku ?? "Producto";
    const color = d.variantes?.colores?.nombre ?? "";
    const talla = d.variantes?.tallas?.nombre ?? "";
    return {
      id: d.id,
